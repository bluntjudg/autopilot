...
const CDP_ENDPOINT = "http://localhost:9224";
...
function checkCDP() {
  return new Promise(resolve => {
    http
      .get(`${CDP_ENDPOINT}/json/version`, res => {
...
async function postOne(page) {
  ...
  await page.keyboard.type(item.generated_comment, { delay: 25 });
...
(async () => {
  console.log("🔗 Connecting to existing Chrome (CDP 9224)");

  const cdpAlive = await checkCDP();
  if (!cdpAlive) {
    console.error("\n❌ Chrome is NOT running on port 9224");
    console.error(
      "👉 google-chrome --remote-debugging-port=9224 --user-data-dir=$HOME/chrome-cdp-seo\n"
    );
    process.exit(1);
  }
